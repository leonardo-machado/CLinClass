(function($, _) { // wrap in anonymous function to not show some helper variables
   ...
   var translations = {
     ...
     ptBR: {
       trash_label: 'Arraste daqui os fragmentos',
       solution_label: 'Construa aqui sua solução',
       order: function() {
         return "Os fragmentos de código em seu programa estão errados, ou na ordem errada. Isso pode ser corrigido movendo, removendo ou substituindo os fragmentos destacados.";},
       lines_missing: function() {
         return "Está faltando alguns fragmentos de código em seu programa.";},
       lines_too_many: function() {
         return "Seu programa tem muitos fragmentos de código.";},
       no_matching: function(lineNro) {
         return "Com base na sintaxe da linguagem, o fragmento em destaque (" + lineNro + ") não está corretamente identado."; },
       no_matching_open: function(lineNro, block) {
         return "O " + block + " fechado na linha " + lineNro + " não foi aberto."; },
       no_matching_close: function(lineNro, block) {
         return "O bloco " + block + " aberto na linha " + lineNro + " não foi fechado";
       },
       block_close_mismatch: function(closeLine, closeBlock, openLine, inBlock) {
         return "Não foi possível fechar o bloco " + closeBlock + " na linha " + closeLine + " quando ainda dentro do bloco " + inBlock + " aberto na linha " + openLine;
       },
       block_structure: function(lineNro) { return "O fragmento destacado "+ lineNro +" pertence a um bloco errado (ex. identação)."; },
       unittest_error: function(errormsg) {
         return "<span class='msg'>Erro na análise/execução de seu programa</span><br/> <span class='errormsg'>" + errormsg + "</span>";
       },
       unittest_output_assertion: function(expected, actual) {
        return "Saída esperada: <span class='expected output'>" + expected + "</span>" +
              "Saída do seu programa: <span class='actual output'>" + actual + "</span>";
       },
       unittest_assertion: function(expected, actual) {
